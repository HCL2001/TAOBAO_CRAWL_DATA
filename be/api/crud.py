...
async def register():
    ...
    url = "https://item.taobao.com/item.htm?id=679086829628&ns=1&abbucket=12#detail"
...
def detailLink( linkDetail: str, product_names: list):
    product_list = []
    headers = {
    ...
    if response.status_code == 200:
        ...
        for name in product_names:
            price = next((key for key in listPrice.keys() if key in name), None)
            promotion = next((key for key in listPromotion.keys() if key in name), None)
            objectProduct = {
                'id': count,
                'name': name,
                'price': listPrice.get(price, ''),
                'promotion': listPromotion.get(promotion, '')
            }
            product_list.append(objectProduct)
            count += 1
        print(product_list)
...
def save_detail(linkDetail: str, product_names: list, output_file: str):
    product_list = detailLink(linkDetail, product_names)

    with open(output_file, 'w') as f:
        json.dump(product_list, f)

# Gọi hàm save_detail để lưu dữ liệu vào tệp JSON
linkDetail = 'https://example.com/detail'
product_names = []  # Thay đổi danh sách tên sản phẩm theo yêu cầu
output_file = 'product_details.json'
save_detail(linkDetail, product_names, output_file)
